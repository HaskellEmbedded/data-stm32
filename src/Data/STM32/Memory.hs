...
ramOffset (_name, mcu) | mcuFamily mcu == F7 = 0x20000000 + (maybe (error "F7 should have mcuCcmRam not Nothing") id $ mcuCcmRam mcu)
...
ram2Offset nmcu@(_name, mcu) | otherwise = maybe Nothing (pure $ Just (ramOffset nmcu + mcuRam1 mcu)) (mcuRam2 mcu)
...
ram3Offset nmcu@(_name, mcu) | otherwise = maybe Nothing (pure $ Just (ramOffset nmcu + mcuRam1 mcu + (maybe 0 id $ mcuRam2 mcu))) (mcuRam3 mcu)
...
ram3Size _    | otherwise = Nothing

rams nmcu@(_name, mcu) =
  [(ramOffset nmcu, mcuRam1 mcu)]
  ++ maybe [] (\x -> [(maybe (error "has mcuRam2 but no ram2Offset") id $ ram2Offset nmcu, x)]) (mcuRam2 mcu)
  ++ maybe [] (\x -> [(maybe (error "has mcuRam3 but no ram3Offset") id $ ram3Offset nmcu, x)]) (mcuRam3 mcu)
