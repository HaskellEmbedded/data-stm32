...
import Ivory.BSP.STM32.Peripheral.SPIv3.Regs
import Ivory.BSP.STM32.Peripheral.SPIv3.Peripheral
...
spiPeripheralDriver tocc periph pins devices req_out res_in ready_in irq watchdog_per = do
  ...
  handler irq "irq" $ do
    callback $ \_ -> do
      ...
      let writeRemaining = tx_sz - tx_pos
          readRemaining  = rx_sz - rx_pos
      ...
      cond_
        [ bitToBool (sr #. spi_sr_rxne) ==> do
            cond_
              [ (readRemaining >=? 2) ==> do
                  comment "still more to receive (more than 2 so use 16 bit write)"
                  r <- spiGetDR16 periph
                  store ((resbuffer ~> rx_buf) ! rx_pos) (bitCast $ r .& 0xFF)
                  store ((resbuffer ~> rx_buf) ! (rx_pos + 1)) (bitCast $ (r `iShiftR` 8) .& 0xFF)
                  store resbufferpos (rx_pos + 2)

              , (readRemaining >? 0) ==> do
                  comment "still more to receive (less than 2 so use 8 bit write)"
                  r <- spiGetDR periph
                  store ((resbuffer ~> rx_buf) ! rx_pos) (bitCast $ r .& 0xFF)
        ...
        , bitToBool (sr #. spi_sr_txe) ==> do
            cond_
              [ (writeRemaining >=? 2) ==> do
                  tx0 <- deref ((reqbuffer ~> tx_buf) ! tx_pos)
                  tx1 <- deref ((reqbuffer ~> tx_buf) ! (tx_pos + 1))
                  spiSetDR16 periph $ (safeCast tx1 `iShiftL` 8) + (safeCast tx0)
                  store reqbufferpos (tx_pos + 2)
              ...
              , (writeRemaining ==? 0) ==> do
                  ...
                  ifte_ (readRemaining >? 0)
                    ...
                    (do
                        assert (readRemaining ==? 0)
                        when (iNot (bitToBool (sr #. spi_sr_bsy))) $ do
  ...
  handler req_out  "request" $ do
    callback $ \req -> do
      ...
      when ready $ do
        ...
        -- Get the first two bytes to transmit
        tx0 <- deref ((reqbuffer ~> tx_buf) ! 0)
        tx1 <- deref ((reqbuffer ~> tx_buf) ! 1)
        store reqbufferpos 2
