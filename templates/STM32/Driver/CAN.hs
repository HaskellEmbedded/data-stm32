...
canTower tocc periph bitrate rxpin txpin = do
  ...
  (apis, transmitters) <- fmap unzip $ forM (canRegTX periph) $ \ txmailbox -> do
  ...
  case apis of
    [api0, api1, api2] -> return (snd reschan, api0, api1, api2)
    _ -> error "Unable to construct canTower with different number of transmitters"
